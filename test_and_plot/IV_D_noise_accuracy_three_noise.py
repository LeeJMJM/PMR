...
def get_inputs_labels(args, snr):
    ...
    dataloaders = {
        ...
        torch.utils.data.DataLoader(
            datasets[phase],
            batch_size=1650,  # 1650 is the length of the test set
...
args.noise_type = 'white'  # 'white' or 'pink' or 'Laplacian'
args.dataset_name = 'EccGear'
matplotlib.use('TkAgg')
if args.noise_type == 'white':
    save_data_path = r'D:\202403_CAMandCNN\1_writing_papers' \
        r'\1_manuscript\plot_codes\acc_white_noise_GearECC.pkl'
elif args.noise_type == 'pink':
    save_data_path = r'D:\202403_CAMandCNN\1_writing_papers' \
        r'\1_manuscript\plot_codes\acc_pink_noise_GearECC.pkl'
elif args.noise_type == 'Laplacian':
    save_data_path = r'D:\202403_CAMandCNN\1_writing_papers' \
        r'\1_manuscript\plot_codes\acc_Laplacian_noise_GearECC.pkl'
else:
    print('args.noise_type is not assigned!')
snr_list = [0, 1, 2, 3, 4, 5]
rep_number = 20
...
for i_snr in snr_list:
    ...
    inputs_data['inputs'], \
        inputs_data['labels'], \
        inputs_data['label_index'] = get_inputs_labels(args, i_snr)
