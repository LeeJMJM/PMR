...
# -*- coding:utf-8 -*-
"""
https://blog.csdn.net/weixin_47174159/article/details/115409058
modefied by Jiaming Li on 2025/1/25
from 2d to 1d
"""
import torch
...
class RSNet(nn.Module):
    ...
    def __init__(self, block, num_block, in_channel=1, out_channel=10):
        super().__init__()

        self.in_channels = 64

        self.conv1 = nn.Conv1d(in_channel, 64,
                               kernel_size=7,
                               stride=2,
                               padding=3,
                               bias=False)
    ...
    def _make_layer(self, block, out_channels_make_layer, num_blocks, stride):
        ...
        Args:
            ...
            out_channels_make_layer: output depth channel number of this layer
        ...
        for stride in strides:
            layers.append(block(self.in_channels, out_channels_make_layer,
                                stride))
            self.in_channels = out_channels_make_layer * block.expansion
    ...
    def forward(self, x):
        ...
        output = self.Iden(output)  # do nothing, only for easy CAM operations
